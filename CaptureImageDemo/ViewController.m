...
@interface ViewController () {
    UIImageView *_zjjImageView;  // 底部图片
...
- (void)creatUI {
    ...
    _zjjImageView.frame = CGRectMake(0, 70, kScreenWidth, kScreenWidth);
    _zjjImageView.userInteractionEnabled = YES;
    [self.view addSubview:_zjjImageView];
    
    UIPanGestureRecognizer *pan = [[UIPanGestureRecognizer alloc]initWithTarget:self action:@selector(panAction:)];
    [_zjjImageView addGestureRecognizer:pan];
...
- (void)movePanAction:(UIPanGestureRecognizer *)gesture {
    ...
    // 底部背景的底部
    float zjj_bottom = _zjjImageView.frame.origin.y+_zjjImageView.frame.size.height;
    // 底部背景的右边
    float zjj_right = _zjjImageView.frame.origin.x+_zjjImageView.frame.size.width;
    // 选取视图的底部
    ...
    // 限制左面
    if (-valueX>=_sizeOrigin_x-_zjjImageView.frame.origin.x) {
        valueX = -(_sizeOrigin_x-_zjjImageView.frame.origin.x);
    }
    // 限制上面
    if (-valueY>=_sizeOrigin_y-_zjjImageView.frame.origin.y) {
        valueY = -(_sizeOrigin_y-_zjjImageView.frame.origin.y);
    }
    // 限制右面
    if (valueX>zjj_right-size_right) {
        valueX = zjj_right-size_right;
    }
    // 限制下面
    if (valueY>zjj_bottom-size_bottom) {
        valueY = zjj_bottom-size_bottom;
    }
...
- (void)panAction:(UIPanGestureRecognizer *)gesture {
    ...
    if (size_bottom > zjj_bottom) {
        _sizeView.frame = CGRectMake(_sizeOrigin_x, _sizeOrigin_y, originX-_sizeOrigin_x, zjj_bottom-_sizeOrigin_y);
    }
}
...
- (void)captureImages {
    ...
    if (sizeViewWidth > 0) {
        // 存在，需要截取选中的图片
        _choseImage = [CustomView captureImageWithView:_zjjImageView andClipRect:CGRectMake(_sizeView.frame.origin.x, _sizeView.frame.origin.y-_zjjImageView.frame.origin.y, sizeViewWidth, sizeViewHeight)];
        _choseImageView.frame = _sizeView.frame;
    }else {
        // 不存在，直接用原图
        _choseImage = _zjjImageView.image;
        _choseImageView.frame = _zjjImageView.frame;
    }
    ...
    for (NSInteger i = 0; i<9; i++) {
        UIImage *image = [CustomView captureImageWithView:_choseImageView andClipRect:CGRectMake((choseImage_Width/3)*(i%3), (choseImage_height/3)*(i/3), choseImage_Width/3, choseImage_height/3)];
